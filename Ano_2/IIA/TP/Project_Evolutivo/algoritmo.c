...
void tournament_geral(pchrom pop, struct info d, pchrom parents) {
    ...
    // Aloca memória para armazenar as posições selecionadas
    pos = malloc(d.tsize * sizeof(int));
    if (pos == NULL) {
        printf("Erro na alocação de memória para pos\n");
        exit(1);
    }

    // Realiza o torneio para selecionar os pais
    for (i = 0; i < d.popsize; i++) {
        // Seleciona tsize soluções diferentes para o torneio
        for (j = 0; j < d.tsize; j++) {
            // Seleciona aleatoriamente uma posição de competidor
            do {
            ...
            } while (k < j); // Se encontrar uma duplicata, tenta novamente
        }

        // Determina o melhor competidor
        best = pos[0];
        for (j = 1; j < d.tsize; j++) {
            if (pop[pos[j]].fitness > pop[best].fitness) {
                best = pos[j];
            }
        }

        // Atribui o melhor indivíduo ao pai selecionado
        parents[i] = pop[best];
    }
